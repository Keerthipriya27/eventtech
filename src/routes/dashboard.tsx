...
function Dashboard() {
  ...
  async function load() {
    ...
    setUser(u);
    const [{ data: prof }, { data: r }, { data: ev }, { data: me }, { data: rg }, { data: tk }, { data: lb }] = await Promise.all([
    ...
    setProfile(prof); setRoles(r?.map(x => x.role) || []); setEvents(ev || []); setMyEvents(me || []); setRegs(rg || []); setTasks(tk || []); setLeaderboard(lb || []);
    setLoading(false);
  }
