...
import {
  Sparkles, LogOut, Calendar, Trophy, Target, Users, Plus, Wand2, Loader2,
  Award, TrendingUp, Zap, BarChart3, QrCode, CheckCircle, Star, Medal,
  Brain, FileText, Network, Smartphone, Shield, Download, Share2,
  UserCheck, ChevronRight, Flame, Crown, Gift, Handshake, GraduationCap,
  ScanLine, ClipboardCheck, Search, Filter, MessageCircle, UserPlus, Bell
} from "lucide-react";
...
function QRImage({ value, size = 160 }: { value: string; size?: number }) {
  ...
  return <img src={url} alt="QR Code" width={size} height={size} className="rounded-lg border border-border" />;
}
...
function Dashboard() {
  ...
  async function load() {
    ...
    try {
      ...
      setLeaderboard(lb || []);
      // Demo notifications
      setNotifications(["🎯 New volunteer task matches your skills!", "🏆 You're #3 on the leaderboard!", "📜 Certificate ready for last event"]);
    } catch (error: any) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
          ...
          <div className="flex items-center gap-2">
            {notifications.length > 0 && (
              <div className="relative">
                ...
                <span className="absolute -top-1 -right-1 w-4 h-4 bg-primary rounded-full text-[10px] flex items-center justify-center text-primary-foreground font-bold">{notifications.length}</span>
              </div>
            )}
            <Badge variant="outline" className="border-primary/30 text-primary hidden sm:flex">
              Lvl {profile?.level ?? 1} · {profile?.xp ?? 0} XP
            </Badge>
            <span className="text-sm text-muted-foreground hidden md:inline">{profile?.full_name || user?.email}</span>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-2xl sm:text-3xl font-bold mb-1">Welcome back, {profile?.full_name?.split(" ")[0] || "there"} 👋</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          ...
          <StatCard icon={Trophy} label="Your XP" value={profile?.xp ?? 0} color="text-yellow-400" />
          <StatCard icon={Target} label="Registrations" value={regs.length} color="text-primary" />
          <StatCard icon={Award} label="Badges" value={profile?.badges?.length ?? 0} color="text-purple-400" />
        </div>
        ...
        <Tabs defaultValue="discover">
          <TabsList className="mb-6 flex-wrap h-auto gap-1 bg-muted/50">
            ...
            {isSponsor && <TabsTrigger value="sponsor">🤝 Sponsor</TabsTrigger>}
          ...
          <TabsContent value="discover"><DiscoverTab events={events} regs={regs} userId={user.id} onRefresh={load} profile={profile} /></TabsContent>
          ...
          <TabsContent value="checkin"><CheckInTab regs={regs} userId={user.id} user={user} profile={profile} /></TabsContent>
          <TabsContent value="network"><NetworkTab events={events} userId={user.id} profile={profile} /></TabsContent>
          <TabsContent value="passport"><PassportTab profile={profile} regs={regs} onSaved={load} /></TabsContent>
          <TabsContent value="leaderboard"><LeaderboardTab lb={leaderboard} userId={user.id} profile={profile} /></TabsContent>
        </Tabs>
      </div>
      <CopilotWidget />
    </div>
...
function DiscoverTab({ events, regs, userId, onRefresh, profile }: any) {
  ...
  const register = async (eventId: string) => {
    ...
    else { toast.success("Registered! 🎉 QR ticket issued."); onRefresh(); }
  };
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <div className="flex gap-2 flex-wrap">
          {categories.slice(0, 5).map(c => (
            <Button key={c} size="sm" variant={filter === c ? "default" : "outline"} onClick={() => setFilter(c)} className="text-xs">{c}</Button>
          ))}
        </div>
      </div>

      {!filtered.length ? <EmptyState label="No events found. Try adjusting your search." /> : (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((e: any) => {
            const pred = predictions[e.id];
            return (
              <Card key={e.id} className="p-5 bg-card border-border hover:border-primary/50 transition-all hover:shadow-lg hover:shadow-primary/5 flex flex-col">
                ...
                <p className="text-sm text-muted-foreground line-clamp-2 mb-3 flex-1">{e.description}</p>
                ...
                {pred && (
                  <div className="mb-3 p-3 rounded-lg bg-primary/5 border border-primary/20 text-xs space-y-1">
                    ...
                    <div className="flex justify-between">
                      ...
                      <Badge variant="outline" className="text-xs py-0">{pred.sentiment}</Badge>
                    </div>
                ...
                <div className="flex flex-col gap-2 mt-auto">
                  {registered.has(e.id) ? (
                    ...
                    <Button size="sm" className="w-full glow-mint" onClick={() => register(e.id)}>Register Now</Button>
                  )}
                  <div className="flex gap-2">
                    <Button size="sm" variant="ghost" className="flex-1 text-xs" onClick={() => predictSuccess(e)} disabled={predicting === e.id}>
                      {predicting === e.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <><Brain className="w-3 h-3 mr-1" />AI Predict</>}
                    </Button>
                    <Button size="sm" variant="ghost" className="flex-1 text-xs" onClick={() => generateProposal(e)} disabled={proposalLoading === e.id}>
                      {proposalLoading === e.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <><FileText className="w-3 h-3 mr-1" />Proposal</>}
                    </Button>
                  </div>
                </div>
              </Card>
      ...
      <Dialog open={!!proposal} onOpenChange={() => setProposal(null)}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader><DialogTitle>📄 AI Sponsorship Proposal — {proposal?.eventTitle}</DialogTitle></DialogHeader>
          <pre className="text-xs whitespace-pre-wrap font-mono bg-muted p-4 rounded-lg leading-relaxed">{proposal?.text}</pre>
          <div className="flex gap-2">
            <Button className="flex-1" onClick={() => {
              const blob = new Blob([proposal?.text || ""], { type: "text/plain" });
              const a = document.createElement("a"); a.href = URL.createObjectURL(blob);
              a.download = `proposal-${proposal?.eventTitle?.replace(/\s+/g, "-")}.txt`; a.click();
              toast.success("Proposal downloaded!");
            }}>
              <Download className="w-4 h-4 mr-2" /> Download
            </Button>
            <Button variant="outline" onClick={() => { navigator.clipboard.writeText(proposal?.text || ""); toast.success("Copied!"); }}>
...
function OrganizeTab({ myEvents, userId, onRefresh }: any) {
  ...
  const create = async () => {
    const payload = {
      ...
      organizer_id: userId, status: "published",
      intelligence_score: aiResult?.intelligence_score || Math.floor(Math.random() * 30) + 60,
      ai_metadata: aiResult || {},
    };
    const { error } = await supabase.from("events").insert(payload);
    if (error) toast.error(error.message);
    else { toast.success("Event launched! 🚀"); setOpen(false); setAiResult(null); onRefresh(); }
  };
  ...
  const runAI = async () => {
    ...
    try {
      const data = await callSupabaseEdgeFn("ai-event-builder", {
        title: form.title, category: form.category, audience: "general", budget: form.budget,
      });
      setAiResult(data);
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button className="glow-mint"><Plus className="w-4 h-4 mr-1" /> New Event</Button></DialogTrigger>
          <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
            <DialogHeader><DialogTitle>Create Event</DialogTitle></DialogHeader>
            <div className="space-y-3">
              <div><Label>Title</Label><Input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="e.g. AI Hackathon 2025" /></div>
              <div><Label>Category</Label>
                <Select value={form.category} onValueChange={v => setForm({ ...form, category: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{["Tech","Hackathon","Workshop","Music","Sports","Business","Arts","Education","Networking"].map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <Button variant="outline" onClick={runAI} disabled={aiLoading} className="w-full border-primary/30">
                {aiLoading ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Wand2 className="w-4 h-4 mr-2" />}
                Generate with AI ✨
              </Button>
              <div><Label>Description</Label><Textarea rows={3} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><Label>Date & Time</Label><Input type="datetime-local" value={form.start_date} onChange={e => setForm({ ...form, start_date: e.target.value })} /></div>
                <div><Label>Location</Label><Input value={form.location} onChange={e => setForm({ ...form, location: e.target.value })} placeholder="City or Online" /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
              ...
              {aiResult && (
                <Card className="p-3 bg-primary/5 border-primary/30">
                  <div className="flex items-center gap-2 mb-2 text-xs text-primary font-medium"><Zap className="w-3 h-3" /> Intelligence Score: {aiResult.intelligence_score}/100</div>
                  {aiResult.tagline && <p className="text-sm italic mb-2 text-muted-foreground">"{aiResult.tagline}"</p>}
                  {aiResult.risks?.length > 0 && <div className="text-xs text-muted-foreground"><strong>Risks:</strong> {aiResult.risks.join(", ")}</div>}
                </Card>
              )}
      ...
      {!myEvents.length ? <EmptyState label="No events yet. Create your first one!" /> : (
        <div className="grid md:grid-cols-2 gap-4">
          {myEvents.map((e: any) => (
            <Card key={e.id} className="p-5 bg-card border-border hover:border-primary/40 transition">
              ...
              <p className="text-xs text-muted-foreground">Event health · {e.intelligence_score}%</p>
            </Card>
          ))}
        </div>
      )}
...
function VolunteerTab({ tasks, userId, onRefresh }: any) {
  ...
  useEffect(() => {
    supabase.from("profiles").select("*").eq("id", userId).maybeSingle().then(({ data }) => setUserProfile(data));
  }, [userId]);
  ...
  const getRecommendations = async () => {
    ...
    try {
      ...
      const scored = openTasks.map((t: any) => {
        ...
        const overlap = userSkills.filter((s: string) => taskSkills.some((ts: string) => ts.includes(s.toLowerCase()) || s.toLowerCase().includes(ts)));
        return { ...t, match_score: Math.min(95, 40 + overlap.length * 25 + Math.floor(Math.random() * 20)), overlap };
      }).sort((a: any, b: any) => b.match_score - a.match_score);
      setRecommended(scored.slice(0, 4));
      toast.success(`🧠 Found ${scored.length} AI-matched tasks!`);
    } catch (e: any) { toast.error(e.message); } finally { setRecLoading(false); }
  };
  ...
  const complete = async (id: string, xp: number) => {
    ...
    toast.success(`🏆 +${xp} XP! Now Level ${newLevel}`);
    onRefresh();
  };
    ...
    <div className="space-y-6">
      ...
      <Card className="p-5 bg-gradient-to-r from-primary/10 to-purple-500/10 border-primary/30">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            <h3 className="font-semibold flex items-center gap-2 mb-1"><Brain className="w-4 h-4 text-primary" /> AI Volunteer Recommender</h3>
            <p className="text-sm text-muted-foreground">Get AI-matched tasks based on your skills: {userProfile?.skills?.join(", ") || "update your passport to get matches"}</p>
          </div>
          <Button onClick={getRecommendations} disabled={recLoading} className="shrink-0">
            {recLoading ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Wand2 className="w-4 h-4 mr-2" />}
            Get Recommendations
          </Button>
        </div>
            ...
            {recommended.map((t: any) => (
              <div key={t.id} className="p-3 rounded-lg bg-background/50 border border-primary/20">
                <div className="flex justify-between mb-1">
                  ...
                  <Badge className="bg-primary/20 text-primary border-primary/30 text-xs">{t.match_score}% match</Badge>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button size="sm" variant="outline" onClick={() => claim(t.id)} className="text-xs h-7">Claim</Button>
                </div>
              </div>
            ))}
          </div>
        ...
        {(["open", "in_progress", "done"] as const).map((col) => (
          <div key={col}>
            ...
            <div className="space-y-2">
              {cols[col].map((t: any) => (
                <Card key={t.id} className="p-3 bg-card border-border hover:border-primary/30 transition">
                  <p className="text-sm font-medium mb-1">{t.title}</p>
                  <p className="text-xs text-muted-foreground mb-2">{t.events?.title}</p>
                  <div className="flex flex-wrap gap-1 mb-2">
                    {t.required_skills?.map((s: string) => <Badge key={s} variant="outline" className="text-xs">{s}</Badge>)}
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-primary font-medium">+{t.xp_reward} XP</span>
                    {col === "open" && <Button size="sm" variant="outline" onClick={() => claim(t.id)} className="text-xs h-7">Claim</Button>}
                    {col === "in_progress" && t.assigned_to === userId && (
                      <Button size="sm" onClick={() => complete(t.id, t.xp_reward)} className="text-xs h-7">✅ Complete</Button>
                    )}
                  </div>
                </Card>
              ))}
              {!cols[col].length && <p className="text-xs text-muted-foreground p-3">Nothing here yet.</p>}
            </div>
          </div>
        ))}
      </div>
  ...
  const runMatch = async () => {
    ...
    try {
      ...
      const localMatches = events.slice(0, 4).map((e: any) => ({
        ...
        recommended_package: budget > 7500 ? "Platinum Partner" : budget > 4000 ? "Gold Sponsor" : "Silver Sponsor",
      }));
  ...
  return (
    <div className="space-y-6">
      <Card className="p-6 bg-card border-border">
        ...
        <p className="text-sm text-muted-foreground mb-5">Our AI analyses your brand profile and finds events with the highest ROI potential.</p>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="p-3 rounded-lg bg-muted text-center"><div className="text-xs text-muted-foreground mb-1">Industry</div><div className="font-semibold text-sm">{industry}</div></div>
          <div className="p-3 rounded-lg bg-muted text-center"><div className="text-xs text-muted-foreground mb-1">Budget</div><div className="font-semibold text-sm">${budget.toLocaleString()}</div></div>
          <div className="p-3 rounded-lg bg-muted text-center"><div className="text-xs text-muted-foreground mb-1">Matches Found</div><div className="font-semibold text-sm">{matches.length}</div></div>
        </div>
      </Card>
      ...
      {matches.length > 0 && (
        <div className="grid md:grid-cols-2 gap-4">
          {matches.map((m: any) => {
            ...
            if (!ev) return null;
            return (
              <Card key={m.event_id} className="p-5 bg-card border-border hover:border-primary/40 transition">
                ...
                <div className="grid grid-cols-2 gap-3 mb-3 text-xs">
                  <div className="p-2 rounded bg-muted">
                    ...
                    <div className="font-semibold text-primary flex items-center gap-1 mt-0.5"><TrendingUp className="w-3 h-3" /> {m.roi_score}/100</div>
                  </div>
                  <div className="p-2 rounded bg-muted">
                    <div className="text-muted-foreground">Impressions</div>
                    <div className="font-semibold flex items-center gap-1 mt-0.5"><BarChart3 className="w-3 h-3" /> {m.predicted_impressions?.toLocaleString()}</div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button size="sm" variant="ghost" className="text-xs" onClick={() => toast.info("Contact organizer to finalize sponsorship!")}>
...
function CheckInTab({ regs, userId, user, profile }: any) {
  const [scanning, setScanning] = useState(false);
  const [checkedIn, setCheckedIn] = useState<Set<string>>(new Set());
  const [certEvent, setCertEvent] = useState<any>(null);

  const doCheckIn = async (eventId: string) => {
    setCheckedIn(prev => new Set([...prev, eventId]));
    // Update registration status in Supabase
    await supabase.from("registrations").update({ checked_in: true, checked_in_at: new Date().toISOString() }).eq("event_id", eventId).eq("user_id", userId);
  ...
  return (
    <div className="space-y-6">
      {/* QR Check-In Section */}
      <Card className="p-6 bg-card border-border">
        <h3 className="font-semibold mb-1 flex items-center gap-2"><QrCode className="w-4 h-4 text-primary" /> QR Event Check-In</h3>
        <p className="text-sm text-muted-foreground mb-5">Show your QR code at the event or scan to check in digitally.</p>
        ...
        {regs.length === 0 ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {regs.map((reg: any) => {
              ...
              return (
                <Card key={reg.id} className={`p-4 border-2 transition-all ${isIn ? "border-green-500/40 bg-green-500/5" : "border-border hover:border-primary/40"}`}>
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="font-medium text-sm leading-tight">{reg.events?.title || "Event"}</h4>
                    {isIn && <Badge className="bg-green-500/20 text-green-500 border-green-500/30 text-xs">✅ Checked In</Badge>}
                  </div>
                  {/* QR Code */}
                  <div className="flex justify-center mb-3">
                    <QRImage value={qrValue} size={120} />
                  </div>
                  <p className="text-xs text-center text-muted-foreground mb-3">Show this QR at event entrance</p>
                  {!isIn && (
                    <Button size="sm" className="w-full mb-2" onClick={() => doCheckIn(reg.event_id)}>
                      <ScanLine className="w-3 h-3 mr-1" /> Check In Now
                    </Button>
                  )}
                  <Button size="sm" variant="outline" className="w-full" onClick={() => downloadCertificate(reg)}>
      ...
      <Card className="p-6 bg-card border-border">
        <h3 className="font-semibold mb-1 flex items-center gap-2"><GraduationCap className="w-4 h-4 text-primary" /> Certificate Automation</h3>
        <p className="text-sm text-muted-foreground mb-4">Certificates are auto-generated for all attended events. Download individually or all at once.</p>
        <div className="space-y-2">
          {regs.map((reg: any) => (
            <div key={reg.id} className="flex items-center justify-between p-3 rounded-lg bg-muted">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Participant · {profile?.full_name || user?.email}</p>
              </div>
          ...
          {regs.length === 0 && <p className="text-sm text-muted-foreground text-center py-4">Register for events to earn certificates.</p>}
        </div>
        {regs.length > 1 && (
          <Button className="w-full mt-4" onClick={() => { regs.forEach((r: any) => setTimeout(() => downloadCertificate(r), 300)); toast.success(`Downloading ${regs.length} certificates!`); }}>
...
function NetworkTab({ events, userId, profile }: any) {
  ...
  const filtered = filter === "All" ? profiles : profiles.filter((p: any) => (p.skills || []).includes(filter));

  return (
    <div className="space-y-6">
      {/* Header */}
      <Card className="p-5 bg-gradient-to-r from-blue-500/10 to-purple-500/10 border-blue-500/20">
        <h3 className="font-semibold mb-1 flex items-center gap-2"><Network className="w-4 h-4 text-blue-400" /> Community Network</h3>
        <p className="text-sm text-muted-foreground">Connect with organizers, volunteers, sponsors, and participants.</p>
      ...
      {allSkills.length > 0 && (
        <div className="flex gap-2 flex-wrap">
          ...
          {allSkills.slice(0, 8).map((s: string) => (
            <Button key={s} size="sm" variant={filter === s ? "default" : "outline"} onClick={() => setFilter(s)} className="text-xs">{s}</Button>
          ))}
        </div>
      )}
      ...
      {loading ? <div className="flex justify-center py-8"><Loader2 className="w-5 h-5 animate-spin text-primary" /></div> : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((p: any) => (
            <Card key={p.id} className="p-4 bg-card border-border hover:border-primary/40 transition">
              <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground truncate">{p.company || p.industry || "EventTech Member"}</p>
                  <Badge variant="outline" className="text-xs mt-1">Lvl {p.level || 1}</Badge>
                </div>
              </div>
              {p.bio && <p className="text-xs text-muted-foreground line-clamp-2 mb-3">{p.bio}</p>}
              {p.skills?.length > 0 && (
                <div className="flex flex-wrap gap-1 mb-3">
                  {p.skills.slice(0, 3).map((s: string) => <Badge key={s} variant="outline" className="text-xs">{s}</Badge>)}
                  {p.skills.length > 3 && <Badge variant="outline" className="text-xs">+{p.skills.length - 3}</Badge>}
                </div>
              )}
              <div className="flex gap-2">
                {connected.has(p.id) ? (
                  <Button size="sm" variant="outline" className="flex-1 text-xs text-green-500 border-green-500/30" disabled>
                    <UserCheck className="w-3 h-3 mr-1" /> Connected
                  </Button>
                ) : (
                  <Button size="sm" variant="outline" className="flex-1 text-xs" onClick={() => connect(p.id, p.full_name)}>
                    <UserPlus className="w-3 h-3 mr-1" /> Connect
                  </Button>
                )}
                <Button size="sm" variant="ghost" className="text-xs px-2" onClick={() => setMsgOpen(p)}>
      ...
      <Dialog open={!!msgOpen} onOpenChange={() => setMsgOpen(null)}>
        <DialogContent className="max-w-sm">
          <DialogHeader><DialogTitle>Message {msgOpen?.full_name}</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <Textarea rows={4} placeholder={`Hi ${msgOpen?.full_name?.split(" ")[0]}, I'd love to connect…`} value={msg} onChange={e => setMsg(e.target.value)} />
            <div className="flex gap-2">
              <Button variant="outline" className="flex-1" onClick={() => setMsgOpen(null)}>Cancel</Button>
              <Button className="flex-1" onClick={sendMessage}><MessageCircle className="w-4 h-4 mr-2" /> Send</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
...
function PassportTab({ profile, regs, onSaved }: any) {
  ...
  const start2fa = async () => {
    ...
    setQrData(res.qr); setPendingSecret(res.base32); setShow2faSetup(true);
  };
  ...
  const do2faConfirm = async () => {
    try {
      ...
      toast.success("🔐 2FA enabled!"); setShow2faSetup(false); setQrData(null); setPendingSecret(null); setTwoFaInput(""); onSaved();
  ...
  return (
    <div className="grid md:grid-cols-2 gap-6">
      <Card className="p-6 bg-card border-border">
        ...
        <div className="space-y-3">
          <div><Label>Bio</Label><Textarea rows={3} value={form.bio} onChange={e => setForm({ ...form, bio: e.target.value })} placeholder="Tell the community about yourself…" /></div>
          <div><Label>Skills (comma-separated)</Label><Input value={form.skills} onChange={e => setForm({ ...form, skills: e.target.value })} placeholder="React, Public Speaking, Marketing…" /></div>
          <div className="grid grid-cols-2 gap-3">
          ...
          <Button onClick={save} className="w-full glow-mint">Save Passport</Button>
      ...
      <div className="space-y-4">
        <Card className="p-6 bg-card border-border">
          ...
          <div className="mb-4">
            <div className="flex justify-between text-sm mb-1">
              ...
              <span className="text-muted-foreground">{profile?.xp ?? 0} / {((profile?.level ?? 1) * 200)} XP</span>
            </div>
          ...
          <h4 className="text-xs uppercase text-muted-foreground mb-2">Events Attended ({regs.length})</h4>
        ...
        <Card className="p-5 bg-card border-border">
          <h3 className="font-semibold mb-3 flex items-center gap-2"><Smartphone className="w-4 h-4 text-primary" /> Mobile-First Profile</h3>
          <p className="text-xs text-muted-foreground mb-3">Your profile QR code — others can scan to connect instantly.</p>
          <div className="flex justify-center">
            <QRImage value={`eventtech-profile:${profile?.id || "user"}:${profile?.full_name || "Member"}`} size={100} />
          </div>
        </Card>
      </div>
      ...
      <Dialog open={show2faSetup} onOpenChange={setShow2faSetup}>
        <DialogContent>
          ...
          <div className="space-y-4">
            {qrData ? <img src={qrData} alt="QR code" className="mx-auto rounded-lg" /> : <p className="text-center text-muted-foreground">Generating QR…</p>}
            <p className="text-sm text-muted-foreground text-center">Scan with Google Authenticator or Authy, then enter the 6-digit code.</p>
            <Input value={twoFaInput} onChange={e => setTwoFaInput(e.target.value)} placeholder="123456" className="text-center text-xl tracking-widest" maxLength={6} />
            <div className="flex gap-2">
              <Button variant="outline" className="flex-1" onClick={() => { setShow2faSetup(false); setQrData(null); setPendingSecret(null); }}>Cancel</Button>
              <Button className="flex-1" onClick={do2faConfirm}>Confirm 2FA</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
...
function LeaderboardTab({ lb, userId, profile }: any) {
  const medals = ["🥇", "🥈", "🥉"];
  const myRank = lb.findIndex((p: any) => p.id === userId) + 1;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Leaderboard */}
      <Card className="p-6 bg-card border-border">
        <h3 className="font-semibold mb-4 flex items-center gap-2"><Trophy className="w-5 h-5 text-yellow-500" /> Top Volunteers</h3>
        <div className="space-y-2">
          {lb.map((p: any, i: number) => (
            <div key={i} className={`flex items-center justify-between p-3 rounded-lg transition ${p.id === userId ? "bg-primary/10 border border-primary/30" : "bg-muted hover:bg-muted/80"}`}>
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${i === 0 ? "bg-yellow-500/20 text-yellow-500" : i === 1 ? "bg-gray-300/20 text-gray-300" : i === 2 ? "bg-amber-700/20 text-amber-500" : "bg-muted text-muted-foreground"}`}>
                  {i < 3 ? medals[i] : i + 1}
                </div>
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-0.5">
                    {(p.badges || []).slice(0, 2).map((b: string) => <Badge key={b} variant="outline" className="text-[10px] py-0 h-4">{b}</Badge>)}
                  </div>
                </div>
              </div>
          ...
          {lb.length === 0 && <p className="text-sm text-muted-foreground text-center py-4">No data yet. Complete volunteer tasks to earn XP!</p>}
        </div>
      </Card>

      {/* XP Guide */}
      <Card className="p-5 bg-card border-border">
        <h3 className="font-semibold mb-3 flex items-center gap-2"><Gift className="w-4 h-4 text-primary" /> How to Earn XP</h3>
